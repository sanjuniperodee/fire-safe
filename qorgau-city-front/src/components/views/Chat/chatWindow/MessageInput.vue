    ...
    <div v-if="!props.selectedChat?.statement || props.selectedChat?.statement?.status !== 'ARCHIVED'" class="message-input">
      ...
      <span class="custom-file-upload">
        <input class="file-input" type="file" accept="application/word, application/vnd.ms-excel, application/vnd.ms-powerpoint,
          ...
          :disabled="isUploadLoading" @change="uploadFile" />
...
const selectInWork = () => {
  emit('updateStatus', 'IN_WORK')
}

const selectComplated = () => {
  emit('updateStatus', 'COMPLETED')
}
...
if (props.selectedChat?.message_set === null) {
  textMessage.value = (`Здравствуйте! Я бы откликнулся на вашу заявку`)
}
