...
SMSC_LOGIN = settings.SMSC_OTP_LOGIN  # логин клиента
SMSC_PASSWORD = settings.SMSC_OTP_PASSWORD  # пароль
...
SMTP_SERVER = settings.SMSC_OTP_HOST  # адрес smtp сервера
...
class SMSC(object):
    ...
    def send_sms(self, phones, message, translit=0, time="", id=0, format=0, sender=False, query=""):
        formats = ["flash=1", "push=1", "hlr=1", "bin=1", "bin=2", "ping=1", "mms=1", "mail=1", "call=1", "viber=1",
                   "soc=1"]
        new_message = f"Код активации {message}\nссылка на сайт https://qorgau-city.kz/"
        m = self._smsc_send_cmd("send", "cost=3&phones=" + quote(phones) + "&mes=" + quote(new_message) + \
